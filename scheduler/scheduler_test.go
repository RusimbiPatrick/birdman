...
func TestGreedySchedulerScoreCandidateNodes(t *testing.T) {
	...
	ts1 := utils.CreateTestServer(utils.NewStatsHandler(node1Stats))
	ts2 := utils.CreateTestServer(utils.NewStatsHandler(node2Stats))
	ts3 := utils.CreateTestServer(utils.NewStatsHandler(node3Stats))
